{-# LANGUAGE DeriveGeneric #-}
{-# LANGUAGE OverloadedStrings #-}

-- | Host-side Breakout model. 'Generic' rows become JS objects via
-- 'JShark.Api.Generic'. Layout matches the MDN canvas workshop grid;
-- 'Game' / 'Brick' / 'Ball' follow the Haskell breakout domain split
-- (state, bricks, ball) without Gloss physics or items.
module JShark.Example.Breakout.Types (module JShark.Example.Breakout.Types) where

import Data.Text (Text)
import GHC.Generics (Generic)

canvasW, canvasH, ballR, paddleW, paddleH, paddleSpeed, paddleMaxX :: Double
canvasW = 480
canvasH = 320
ballR = 10
paddleW = 75
paddleH = 10
paddleSpeed = 7
paddleMaxX = canvasW - paddleW

brickW, brickH, brickPad, brickOffL, brickOffT :: Double
brickW = 75
brickH = 20
brickPad = 10
brickOffL = 30
brickOffT = 30

brickColumnCount, brickRowCount, brickCount, startLives :: Int
brickColumnCount = 5
brickRowCount = 3
brickCount = brickColumnCount * brickRowCount
startLives = 3

boardId, ballFill, ink, bannerFill, boardFill :: Text
boardId = "board"
ballFill = "#0369a1"
ink = "#0c4a6e"
bannerFill = "#0f172a"
boardFill = "#e2e8f0"

data Phase = Play | Win | Lose deriving (Eq, Generic)

data Ball = Ball {x, y, dx, dy :: Double} deriving Generic

data Paddle = Paddle {px :: Double} deriving Generic

data Brick = Brick {bx, by :: Double, alive :: Bool, color :: Text}
  deriving Generic

data Game = Game
  { ball :: Ball
  , paddle :: Paddle
  , bricks :: [Brick]
  , score, lives :: Int
  , phase :: Phase
  , leftOn, rightOn :: Bool
  }
  deriving Generic

startBall :: Ball
startBall = Ball (canvasW / 2) (canvasH - 30) 2 (-2)

startPaddle :: Paddle
startPaddle = Paddle (paddleMaxX / 2)

-- | Rows top to bottom, colored dark to light.
layoutBricks :: [Brick]
layoutBricks = liftA2 brick [0 .. brickRowCount - 1] [0 .. brickColumnCount - 1]
 where
  rowColors = ["#0c4a6e", "#0369a1", "#0284c7", "#0ea5e9", "#38bdf8"]
  brick row col =
    Brick
      { bx = fromIntegral col * (brickW + brickPad) + brickOffL
      , by = fromIntegral row * (brickH + brickPad) + brickOffT
      , alive = True
      , color = cycle rowColors !! row
      }

startGame :: Game
startGame =
  Game startBall startPaddle layoutBricks 0 startLives Play False False
