  ...
  hold $
    ffi
      ( "(() => {"
          <> "let worker = null;"
          <> "try {"
          <> "  const src = 'self.onmessage=function(e){var a=new Uint8Array(e.data.a),s=new Uint8Array(e.data.s),c=new Uint32Array(256);for(var i=0,n=a.length;i<n;i++)if(a[i]===1)c[s[i]]++;self.postMessage(c,[c.buffer]);};';"
          <> "  const url = URL.createObjectURL(new Blob([src],{type:'text/javascript'}));"
          <> "  worker = new Worker(url);"
          <> "  URL.revokeObjectURL(url);"
          <> "} catch (e) {}"
          <> "return { lastMs: 0, counts: new Uint32Array(256), rows: new Map(), order: [], worker, pending: false };"
          <> "})"
      )
      RecNil
  ...
  hold $
    ffi
      ( "((hostId, listId) => {"
          ...
          <> "  style.textContent = '"
          <> ".life-index-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));"
          <> "gap:0.35rem 2rem;font-size:0.85rem;color:#94a3b8;text-align:left;contain:layout style}"
...
-- | Count off the animation frame (Worker, or setTimeout fallback) so
--   Chrome/Firefox do not share a long task with Conway + blit. Each
--   refresh patches count text and reorders existing row nodes when
--   rank changes — no innerHTML, no CSS @order@, no class churn unless
--   a row dies or revives.
...
stepIndexTrackerJs =
  "((alive, species, palette, registry, tracker, seen, container, now, refreshMs, soupSid, manualSid) => {"
    ...
    ++ "if (tracker.lastMs !== 0 && now - tracker.lastMs < refreshMs) return;"
    ...
    ++ "  const base = sid * 3;"
    ++ "  swatch.style.background = 'rgb(' + palette[base] + ',' + palette[base + 1] + ',' + palette[base + 2] + ')';"
    ++ "  const nameEl = document.createElement('span');"
    ++ "  nameEl.className = 'index-name';"
    ++ "  nameEl.textContent = displayName(sid);"
    ++ "  const countEl = document.createElement('span');"
    ++ "  countEl.className = 'index-count';"
    ++ "  row.append(swatch, nameEl, countEl);"
    ++ "  row._countEl = countEl;"
    ++ "  row._cnt = -1;"
    ++ "  row._dead = false;"
    ++ "  tracker.rows.set(sid, row);"
    ++ "  return row;"
    ++ "};"
    ++ "const patchCounts = (counts) => {"
    ++ "  for (let sid = 0; sid < 256; sid++) {"
    ++ "    if (counts[sid] !== 0) seen.add(sid);"
    ++ "  }"
    ...
    ++ "    if (row._cnt !== cnt) {"
    ++ "      row._cnt = cnt;"
    ++ "      row._countEl.textContent = '' + cnt;"
    ++ "      const dead = cnt === 0;"
    ++ "      if (row._dead !== dead) {"
    ++ "        row._dead = dead;"
    ++ "        row.className = dead ? 'index-row index-row-dead' : 'index-row';"
    ++ "      }"
    ++ "    }"
    ++ "  }"
    ++ "};"
    ++ "const sortRows = (counts) => {"
    ++ "  const order = [];"
    ++ "  for (const sid of seen) order.push(sid);"
    ++ "  order.sort((a, b) => {"
    ++ "    const d = counts[b] - counts[a];"
    ++ "    return d !== 0 ? d : a - b;"
    ++ "  });"
    ++ "  const prev = tracker.order;"
    ++ "  let same = prev.length === order.length;"
    ++ "  if (same) {"
    ++ "    for (let i = 0; i < order.length; i++) {"
    ++ "      if (prev[i] !== order[i]) { same = false; break; }"
    ++ "    }"
    ++ "  }"
    ++ "  if (same) return;"
    ++ "  const frag = document.createDocumentFragment();"
    ++ "  const rows = tracker.rows;"
    ++ "  for (let i = 0; i < order.length; i++) frag.appendChild(rows.get(order[i]));"
    ...
    ++ "  }"
    ++ "};"
    ++ "const countInto = (srcAlive, srcSpecies, out) => {"
    ++ "  out.fill(0);"
    ++ "  for (let i = 0, n = srcAlive.length; i < n; i++) {"
    ++ "    if (srcAlive[i] === 1) out[srcSpecies[i]]++;"
    ++ "  }"
    ++ "};"
    ++ "if (tracker.worker) {"
    ++ "  const a = alive.slice();"
    ++ "  const s = species.slice();"
    ++ "  const onMsg = (e) => {"
    ++ "    tracker.worker.removeEventListener('message', onMsg);"
    ++ "    apply(new Uint32Array(e.data));"
    ++ "  };"
    ++ "  tracker.worker.addEventListener('message', onMsg);"
    ++ "  try {"
    ++ "    tracker.worker.postMessage({a: a.buffer, s: s.buffer}, [a.buffer, s.buffer]);"
    ++ "  } catch (err) {"
    ++ "    tracker.worker.removeEventListener('message', onMsg);"
    ++ "    countInto(alive, species, tracker.counts);"
    ++ "    apply(tracker.counts);"
    ++ "  }"
    ++ "} else {"
    ++ "  countInto(alive, species, tracker.counts);"
    ++ "  const snap = tracker.counts;"
    ++ "  setTimeout(() => apply(snap), 0);"
    ++ "}"
    ++ "})"
