  ...
  testGroup
    ...
    [ -- Measured on GHC 9.14.1 / -O2 after Index-only Array.index + dropped
      ...
      testCase "Life raw IR nodes" $ do
        ...
        assertCeiling "rawNodes" n 72000
    , -- Process-wide Life metrics run allocated ~7.01 GB. Cap optimize
      ...
      testCase "Life optimize nodes and alloc" $ do
        (n, bytes) <- allocated (evaluate (optimizedEffectSize life))
        assertCeiling "optNodes" n 110000
        assertCeiling "optAlloc" bytes 8000000000
    , probeCase 16 3301 10000000
    , probeCase 32 11005 70000000
    , -- Life output is deterministic; cap UTF-8 bytes and hoisted
      -- `$`-helpers so an accidental helper explosion or inline regression
      -- trips here as well as in the golden test.
      testCase "Life output bytes and helper count" $ do
        let
          js = renderJS (effectfulAST life)
        assertCeiling "lifeBytes" (BS.length js) 900000
        assertCeiling "lifeHelpers" (T.count "const $" (TE.decodeUtf8 js)) 100
    ]
...
probeCase n maxChars maxAlloc =
  testCase ("probe " ++ show n ++ " JS size and alloc") $ do
    (chars, bytes) <-
      allocated (evaluate (BS.length (renderJS (effectfulAST (probeN n)))))
...
-- | Bytes allocated by an action (the counter counts down).
