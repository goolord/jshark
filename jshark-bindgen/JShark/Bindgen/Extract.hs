...
-- | Locate @jshark-bindgen/extract.mjs@: env, cabal data file, CWD, or parents.
...
findExtractScript = do
 ...
 where
  search = do
    installed <-
      try (getDataFileName "jshark-bindgen/extract.mjs") ::
  ...
  walkCwd = do
    ...
    firstExist
      ( [ cwd </> "jshark-bindgen" </> "extract.mjs"
        , cwd </> "extract.mjs"
        ]
          <> [ walk </> "jshark-bindgen" </> "extract.mjs"
             | walk <- parents cwd
             ]
      )
