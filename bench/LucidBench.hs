...
import Lucid (div_, li_, label_, button_, class_, type_)
...
largeTemplate n = div_ [class_ "container"] $ 
  mapM_ (\i -> todoRow (string ("Todo " <> T.pack (show i))) (bool (i `mod` 2 == 0))) [1..n]
...
timeIt label action = do
  ...
  let diff = fromIntegral (end - start) / 1e9 :: Double
  printf "%s: %0.3f ms\n" label diff
  return res
...
main = do
  ...
  let test n = do
        putStrLn $ "\nTesting n=" ++ show n
        timeIt "  optimized" $ do
          let !size = optimizedEffectSize (benchmarkTemplate n)
          printf "    size: %d\n" size
        
        timeIt "  unoptimized" $ do
          let !js = renderJSCompact (effectfulAST (benchmarkTemplate n))
          printf "    js length: %d\n" (T.length js)
