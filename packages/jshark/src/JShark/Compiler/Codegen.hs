 ...
 where
  go p acc =
    let
      (hs, acc') =
        foldr
          (\c (hs', a) -> let (hc, a') = go c a in (hc : hs', a'))
          ([], acc)
          (toList (pN p))
...
plan st env (P i _ n) = case n of
  ...
  NLet t h x b -> do
    ...
    ret [v] (NLet t h x' b')
  NLetRec t r b ->
    new Nothing >>= \v -> ret [v] =<< (NLetRec t <$> goWith [(t, v)] r <*> goWith [(t, v)] b)
  NBindRec t r b ->
    new Nothing >>= \v -> ret [v] =<< (NBindRec t <$> goWith [(t, v)] r <*> goWith [(t, v)] b)
  NLam t info b ->
    inScope $
      new (lamParam info) >>= \v -> ret [v] . NLam t info =<< goWith [(t, v)] b
  ...
  NMeth m a z ts b -> do
    ...
    inScope $ do
      let
        k = if m == MFrom then 2 else length ts
      vs <- drop (k - length ts) <$> replicateM k (new Nothing)
      ret vs . NMeth m a' z' ts =<< goWith (zip ts vs) b
  NBind t h x b
    | P _ _ (NLift (P _ _ (NVar j))) <- b
    , j == t ->
        go x >>= \x' -> ret [] . NBind t h x' =<< go b
    | otherwise -> do
        x' <- go x
        v <- new h
...
emit table s0 (P _ ann n) = case n of
  ...
  NLet t _ x b
    ...
    , yc <- c b ->
        same
          ( keepRef
              ( fromMaybe mempty xd
                  $$ constBind bound (fromMaybe mempty xr)
                  $$ fromMaybe mempty (codeDecl yc)
              )
              yc
          )
  NLetRec _ r b
    | MkCode rd rr _ <- c r
    , bc <- c b ->
        same (keepRef (recBindStmt bound rd rr $$ fromMaybe mempty (codeDecl bc)) bc)
  ...
  NApp f x ->
    let
      (hd, args) = collect f [x]
     in
      if length args > 1 && callArity hd == length args
        then
          let
            Code fd fr = c hd; cs = map c args
           in
            same
              ( Code
                  (foldl' ($$) fd [d | Code d _ <- cs])
                  (parens fr <> parens (hcat (intersperse ", " [r | Code _ r <- cs])))
              )
        else
          let
            Code fd fr = c f; Code xd xr = c x
           in
            same (Code (fd $$ xd) (parens fr <> parens xr))
  NAppE f x
    | Code fd fr <- c f
    , Code xd xr <- c x ->
        same (fxCode (fd $$ xd) (parens fr <> parens xr))
  ...
  NIfE cnd t e
    ...
    , MkCode ed er _ <- c e ->
        same
          ( MkCode
              (Just (fromMaybe mempty cd $$ ifElseStmt (fromMaybe mempty cr) td tr ed er))
              Nothing
              False
          )
      ...
      body =
        fromMaybe mempty od
          ...
          $$ ifAssignOrStmt
            (Just rv)
            (jsText ov <+> ".some")
            (Just (constBind bound (jsText ov <> ".value") $$ fromMaybe mempty sd))
            sr
            nd
            nr
     in
  ...
  NOptCaseE o x _ s ->
    let
      ...
      pre =
        fromMaybe mempty od
          $$ constBind ov (fromMaybe "null" orf)
          $$ constBind bound (jsText ov <> ".value")
      MkCode nd nr _ = c x
      MkCode sd sr _ = c s
     in
     ...
     in
      if isNothing ed && isNothing od
        then
          ...
          , MkCode
              (Just pre)
              (Just (parens (cond <+> "?" <+> undef orf <+> ":" <+> undef er)))
              False
          )
        else
          let
           ...
           in
            ( s2
            , MkCode
                (Just (pre $$ letResult rv $$ ifAssignOrStmt (Just rv) cond od orf ed er))
                (Just (jsText rv))
                False
            )
  NResCaseE o _ e _ k ->
          ...
          let
            ...
            stmt =
              pre
                ...
                $$ ifElseStmt
                  cond
                  (Just (fromMaybe mempty od $$ assignResult rv orf))
                  Nothing
                  (Just (fromMaybe mempty ed $$ assignResult rv er))
                  Nothing
           in
  ...
  NFnLit _ _ b
    | Code d r <- c b ->
        same (Code mempty (renderFn (map jsText ann) (nonEmpty d) (Just r)))
  NNullable x
    | Code d r <- c x ->
        same
          ( Code
              d
              ("((v) => v == null ? {some: false} : {some: true, value: v})" <> parens r)
          )
  ...
  NK2 op x y
    | Code xd xr <- c x
    , Code yd yr <- c y ->
        same (Code (xd $$ yd) (operand x xr <+> jsText (op2JS op) <+> operand y yr))
     ...
     in
      same $ case (m, z) of
        ...
        (MFrom, _) ->
          Code
            ad
            ("Array.from({length: " <> ar <> "}, " <> cb ("_" : map jsText ann) <> ")")
        (_, Just zc)
          | Code zd zr <- c zc ->
              let
                method = if m == MReduce then ".reduce" else ".reduceRight"
               in
                Code (ad $$ zd) (recv <> method <> parens (cb (map jsText ann) <> ", " <> zr))
        _ -> error "JShark.Compiler.Codegen: reduce without a seed"
  NFFI form args ->
    let cs = map c args in same (fxCode (decls cs) (renderFFIInvoke form (refs cs)))
  NUObj t -> same (Code mempty (jsText t))
  NUSet x y
    | Code xd xr <- c x
    , Code yd yr <- c y ->
        same (fxCode (xd $$ yd) (xr <> " = " <> yr))
  NCall o m args
    | Code od orf <- c o
    , cs <- map c args ->
  ...
  NBind t _ x b
    ...
    , MkCode yd yr yfx <- c b ->
        same $ case xr of
          ...
          Just r ->
            MkCode
              (Just (fromMaybe mempty xd $$ constBind bound r $$ fromMaybe mempty yd))
              yr
              yfx
  NThen x y
    | MkCode xd xr xfx <- c x
          ...
          same . (\js -> MkCode (Just js) Nothing False) $ case cd of
            ...
            Just d ->
              "while"
                <+> parens "true"
                <+> blockBody
                  ((d $$ ("if" <+> parens ("!" <> parens cjs) <+> blockBody "break;")) $$ body)
  NFor a e _ b
    | MkCode ad ar _ <- c a
    , MkCode ed er _ <- c e
    , MkCode bd br _ <- c b ->
        let
          ...
          hd =
            hcat
              [ ("let" <+> v <+> "=" <+> fromMaybe mempty ar)
              , ";"
              , " "
              , (v <+> "<" <+> fromMaybe mempty er)
              , ";"
              , " "
              , v <> "++"
              ]
          ...
          same
            ( MkCode
                ( Just
                    ( fromMaybe mempty ad
                        $$ fromMaybe mempty ed
                        $$ ("for" <+> parens hd <+> blockBody (asStmt bd br))
                    )
                )
                Nothing
                False
  ...
  NU8Set a i v
    ...
    , Code vd vr <- c v ->
        same
          (Code (ad $$ idd $$ vd $$ (((ar <> brackets ir) <+> "=" <+> vr) <> semi)) mempty)
  NU8Fill a v
    | Code ad ar <- c a
    , Code vd vr <- c v ->
        same (Code (ad $$ vd $$ ((ar <> ".fill" <> parens vr) <> semi)) mempty)
  NStrCase o arms d ->
    let
      ...
      cases =
        [ "case" <+> (jsQuote k <> colon) <+> blockBody (arm e <+> ("break" <> semi))
        | (k, e) <- arms
        ]
      sw =
        "switch"
          <+> parens orf
          <+> blockBody (vcat (cases ++ ["default:" <+> blockBody (arm d)]))
      pre = if unit then od else od $$ letResult rv
     in
      same
        (MkCode (Just (pre $$ sw)) (if unit then Nothing else Just (jsText rv)) False)
  ...
  NDelete o k
    | Code od orf <- c o
    , Code kd kr <- c k ->
        same (fxCode (od $$ kd) (("delete" <+> orf) <> brackets kr))
 ...
 where
    ...
    let
     ...
     in
      if isNothing td && isNothing ed && not tfx && not efx
        then
          same
            (MkCode cd' (Just (parens (cjs <+> "?" <+> undef tr <+> ":" <+> undef er))) False)
        else
          let
            (rv, s1) = tmp s0
           in
            ...
            , MkCode
                ( Just
                    (fromMaybe mempty cd $$ letResult rv $$ ifAssignOrStmt (Just rv) cjs td tr ed er)
                )
                (Just (jsText rv))
                False
            )
  ...
  branching unit s1 pre k
    ...
    | otherwise =
        let
         ...
         in
          ( s2
          , MkCode (Just (pre $$ letResult rv $$ k (Just rv))) (Just (jsText rv)) False
          )
  ...
  resPrelude o =
    let
     ...
     in
      ...
      , s1
      , fromMaybe mempty od
          $$ constBind obj (fromMaybe mempty orf)
          $$ constBind unw (jsText obj <> ".value")
      )
      ...
      case c y of
        MkCode Nothing yr _ ->
          same
            ( MkCode
                xd
                (Just (xjs <+> (if isAnd then "&&" else "||") <+> operand y (undef yr)))
                False
            )
        MkCode (Just yd) yr _ ->
          let
           ...
           in
            ...
            , MkCode
                ( Just
                    (fromMaybe mempty xd $$ xInit $$ ("if" <+> parens cond <+> blockBody guard))
                )
                (Just v)
                False
            )
  ...
  valueEq wrap x y
    ...
    , Code yd yr <- c y =
        let
          (s1, js) = shim ValueEq [operand x xr, operand y yr] s0
         in
          (s1, Code (xd $$ yd) (wrap js))
    ...
    let
     ...
     in
      Code
        ...
        (braces (hcat (intersperse ", " (map snd parts))))
...
-- | @new Uint8Array(n)@ when all bytes are zero, else the element list.
bytesLit :: JS -> ByteArray -> JS
bytesLit ctor ba =
  let
    es = uint8Elems ba
   in
    "new "
      ...
      <> parens
        ( if all (== 0) es
            then jsDecimal (length es)
            else brackets (hcat (intersperse ", " (map jsDecimal es)))
        )

resultObject :: Bool -> Maybe JS -> JS
resultObject isOk payload =
  braces
    ( (("ok:" <+> (if isOk then "true" else "false")) <> ",")
...
tryCatchStmt mr v ad ar bd br = case mr of
  Nothing ->
    "try" <+> blockBody (asStmt ad ar) $$ (catchHead <+> blockBody (asStmt bd br))
  Just rv ->
    "try"
      <+> blockBody (fromMaybe mempty ad $$ assignResult rv ar)
...
renderFFIInvoke fn args = case fn of
  ...
  FFIExpr s -> jsText s
  FFICall s
    | "=>" `T.isInfixOf` s && not (wholeParens s) ->
        parens (jsText s) <> parens args
